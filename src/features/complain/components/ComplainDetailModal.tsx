...
const ComplainDetailModal: React.FC<ComplainDetailModalProps> = ({ complaint, onClose, onUpdateComplaint, mode = 'my' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white rounded border border-gray-300 shadow-2xl w-full max-w-5xl flex flex-col max-h-[90vh] overflow-hidden">
        
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50/50">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-red-50 text-[#A31736] rounded border border-red-100">
              <FileTextIcon />
            </div>
            <div>
              <h2 className="text-lg font-bold text-gray-900 uppercase tracking-tight">
                Complaint Details - <span className="text-[#A31736]">{complaint.refId}</span>
              </h2>
              <p className="text-xs text-gray-500">Pradeshiya Sabha Citizen Grievance Docket</p>
            </div>
          </div>
          <button 
            type="button"
            onClick={onClose}
            className="text-gray-400 hover:text-gray-700 hover:bg-gray-100 p-2 rounded transition-colors cursor-pointer"
          >
            <CloseIcon />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-gray-50/50 border border-gray-200 rounded p-4">
              ...
              <div className="space-y-2.5 text-xs">
                <div className="grid grid-cols-3">
                  ...
                  <span className="col-span-2 font-bold text-gray-900">: {complaint.citizenName}</span>
                </div>
                <div className="grid grid-cols-3">
                  <span className="text-gray-500">NIC Number</span>
                  <span className="col-span-2 font-semibold text-gray-900">: {complaint.citizenNic}</span>
                </div>
                <div className="grid grid-cols-3">
                  <span className="text-gray-500">Phone</span>
                  <span className="col-span-2 font-semibold text-gray-900">: {complaint.citizenPhone}</span>
                </div>
                <div className="grid grid-cols-3">
                  <span className="text-gray-500">Email</span>
                  <span className="col-span-2 font-semibold text-gray-900">: {complaint.citizenEmail}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50/50 border border-gray-200 rounded p-4">
              ...
              <div className="space-y-2.5 text-xs">
                <div className="grid grid-cols-3 items-center">
                  ...
                  <span className="col-span-2 font-bold text-gray-900">: {complaint.category}</span>
                </div>
                <div className="grid grid-cols-3 items-center">
                  <span className="text-gray-500">Date & Time</span>
                  <span className="col-span-2 font-semibold text-gray-900">: {complaint.date} | {complaint.time}</span>
                </div>
                <div className="grid grid-cols-3 items-center">
                  <span className="text-gray-500">Assigned Officer</span>
                  <span className="col-span-2 font-bold text-gray-900">: {complaint.assignedOfficer}</span>
                </div>
                <div className="grid grid-cols-3 items-center">
                  <span className="text-gray-500">Status</span>
                  <div className="col-span-2 flex items-center gap-1">
                    <span>: </span>
                    <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${getStatusStyle(currentStatus)}`}>
                      {currentStatus}
                    </span>
                  </div>
                </div>
                {assignedTech && (
                  <div className="grid grid-cols-3 items-center">
                    ...
                    <span className="col-span-2 font-bold text-blue-700 flex items-center gap-1.5">
                      <span>: </span>
                ...
                <div className="grid grid-cols-3 items-center">
                  ...
                  <span className="col-span-2 font-bold text-gray-900">: {complaint.dueDate || '—'}</span>
                </div>
                {(() => {
                  ...
                  return dlStatus ? (
                    <div className="grid grid-cols-3 items-center">
                      ...
                      <div className="col-span-2 flex items-center gap-1.5">
                        <span>: </span>
                        <span className={`px-2 py-0.5 rounded border text-[10px] font-bold uppercase tracking-wider inline-flex items-center gap-1 ${getDeadlineStatusStyleClasses(complaint.dueDate || '')}`}>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-2">
                {complaint.attachments.length === 0 ? (
                  ...
                  complaint.attachments.map(att => (
                    <div key={att.id} className="flex items-center justify-between p-2.5 bg-white border border-gray-200 rounded">
                      <div className="flex items-center gap-3">
                        <div className="p-1.5 bg-gray-50 rounded border border-gray-200">
                          {att.type === 'image' ? <ImageIcon /> : <PdfIcon />}
                        </div>
                        <div>
                          <p className="text-xs font-bold text-gray-900">{att.name}</p>
                          <p className="text-[10px] text-gray-500">{att.size}</p>
                        </div>
                      </div>
                      <button type="button" className="p-1.5 bg-white text-[#A31736] border border-gray-300 rounded hover:bg-red-50 transition-colors shadow-sm cursor-pointer">
                        <DownloadIcon />
                      </button>
                    </div>
                  ))
            ...
            <div className="bg-white border border-gray-300 rounded p-5 relative overflow-hidden shadow-sm">
              {notification && (
                <div className="mb-3 p-2.5 bg-green-100 border border-green-300 text-green-800 rounded text-xs font-bold flex items-center justify-between animate-fade-in">
              ...
              {currentStatus === 'COMPLETED' && (
                <div className="mb-4 p-3.5 bg-green-50 border border-green-200 rounded flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fade-in shadow-sm">
                  <div className="flex items-center gap-2.5">
                    <div className="w-7 h-7 rounded bg-green-600 text-white flex items-center justify-center shrink-0 shadow">
                    ...
                    <div>
                      <div className="text-xs font-bold text-green-950 uppercase">Grievance Resolved & Completed</div>
                      <div className="text-[11px] text-green-800">Citizen: {complaint.citizenName} ({complaint.citizenPhone})</div>
                    </div>
                  </div>
                  <button
                    type="button"
                    ...
                    className={`px-3.5 py-1.5 rounded text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 shrink-0 cursor-pointer shadow-sm ${
                      citizenNotified
                        ? 'bg-green-800 text-white opacity-90 cursor-default'
              ...
              {mode === 'all' ? (
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="flex items-center gap-2 text-xs font-bold text-gray-800 uppercase tracking-wider">
                    ...
                    <span className="text-[10px] font-bold bg-gray-100 text-gray-700 px-2 py-0.5 rounded border border-gray-200 uppercase">
                  ...
                  <div className="flex gap-2 mb-3">
                    <input
                      ...
                      className="flex-1 px-3 py-2 bg-white border border-gray-300 rounded text-xs focus:outline-none focus:border-[#A31736]"
                      onKeyDown={(e) => {
                    ...
                    <button
                      ...
                      className="bg-[#A31736] text-white px-4 py-2 rounded text-xs font-bold uppercase tracking-wider hover:bg-[#801028] transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0 cursor-pointer shadow-sm"
                    >
                      Send
                    </button>
                  </div>
                  ...
                  {remarksList.length > 0 ? (
                    <div className="space-y-2 mt-3 max-h-44 overflow-y-auto pr-1">
                      ...
                      {remarksList.map((rem) => (
                        <div key={rem.id} className="p-2.5 bg-gray-50 border border-gray-200 rounded text-xs space-y-1">
                          <div className="flex items-center justify-between font-bold text-gray-700">
                            <span className="text-[#A31736]">{rem.author} → {complaint.assignedOfficer}</span>
                    ...
                    <div className="p-3 bg-gray-50 border border-dashed border-gray-200 rounded text-center">
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {assignedTech && (
                      <span className="text-[11px] font-bold bg-blue-100 text-blue-800 px-2.5 py-0.5 rounded border border-blue-200">
                        Tech: {assignedTech}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex gap-3">
                    <button 
                      ...
                      className={`flex-1 font-bold py-2.5 rounded shadow-sm transition-colors cursor-pointer flex items-center justify-center gap-2 text-xs uppercase tracking-wider ${
                        isAssigningTech ? 'bg-[#801028] text-white' : 'bg-[#A31736] text-white hover:bg-[#801028]'
                      }`}
                    >
                      <span>Assign Technician</span>
                      <span className="text-[10px] opacity-80">{isAssigningTech ? '▲' : '▼'}</span>
                    </button>
                    <button 
                      type="button"
                      onClick={() => { setIsChangingState(!isChangingState); setIsAssigningTech(false); }}
                      className={`flex-1 border font-bold py-2.5 rounded transition-colors cursor-pointer flex items-center justify-center gap-2 text-xs uppercase tracking-wider ${
                        isChangingState ? 'bg-red-50 text-[#A31736] border-[#A31736]' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                      }`}
                    >
                      <span>Change State</span>
                      <span className="text-[10px] opacity-80">{isChangingState ? '▲' : '▼'}</span>
                    </button>
                  </div>
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left p-2.5 rounded border transition-all flex items-center justify-between cursor-pointer ${
                                isSelected
                                  ? 'bg-red-50 border-[#A31736] text-[#A31736] font-bold shadow-sm'
                            ...
                            >
                              <div>
                                <div className="text-xs font-bold flex items-center gap-1.5">
                                  <span>{tech.name}</span>
                                  {isSelected && <span className="text-[9px] bg-[#A31736] text-white px-1.5 py-0.2 rounded font-extrabold">ASSIGNED</span>}
                                </div>
                                <div className="text-[11px] text-gray-500 font-normal mt-0.5">{tech.role} • {tech.dept}</div>
                              </div>
                          ...
                          return (
                            <button
                              ...
                              className={`p-2.5 rounded border text-left flex items-center gap-2.5 transition-all cursor-pointer ${
                                isSelected
                                  ? 'bg-red-50 border-[#A31736] font-bold text-[#A31736] shadow-sm'
                                  : 'bg-white border-gray-200 hover:bg-gray-50 text-gray-700'
                              }`}
                            >
                              <span className={`w-2.5 h-2.5 rounded-full ${opt.color} shrink-0`}></span>
                              <span className="text-xs font-semibold">{opt.label}</span>
                              {isSelected && <span className="ml-auto text-xs text-[#A31736]">✓</span>}
                            </button>
                          )
                        })}
                      </div>
        ...
        {/* Footer */}
        <div className="px-6 py-3.5 border-t border-gray-200 bg-gray-50/70 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <button 
            type="button"
            onClick={onClose}
            className="bg-gray-800 text-white text-xs font-bold uppercase tracking-wider px-6 py-2 rounded hover:bg-gray-900 transition-colors cursor-pointer shadow-sm"
          >
            Close Detail View
          </button>
        </div>

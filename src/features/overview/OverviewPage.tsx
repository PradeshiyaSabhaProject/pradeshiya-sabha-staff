    ...
    <div className="space-y-6 animate-fade-in pb-8">
          ...
          : kpiStats.map(stat => (
            ...
            >
              <div className="flex items-start justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-gray-600 truncate mr-2">{stat.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <table className="w-full text-left border-collapse min-w-[580px]">
              <thead>
                <tr className="bg-gray-100 border-y border-gray-300 text-[11px] font-bold text-gray-600 uppercase tracking-wider">
                  <th className="py-3 px-4 sm:px-6">REF ID</th>
                  <th className="py-3 px-4 sm:px-6">DEPARTMENT</th>
                  <th className="py-3 px-4 sm:px-6">TASK SUBJECT</th>
              ...
              <tbody className="divide-y divide-gray-200 text-sm">
                {loading ? (
                  [1, 2, 3, 4, 5, 6].map(i => (
                    <tr key={i}>
                      <td colSpan={5} className="py-4 px-4 sm:px-6">{skeleton('h-8')}</td>
                    </tr>
                  ))
                ) : (
                  taskInbox?.map(item => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                      <td className="py-3 px-4 sm:px-6 font-mono text-xs font-semibold text-gray-600 whitespace-nowrap">{item.refId}</td>
                      <td className="py-3 px-4 sm:px-6 font-bold text-gray-900 whitespace-nowrap">{item.department}</td>
                      <td className="py-3 px-4 sm:px-6 text-gray-800 max-w-[200px] sm:max-w-sm truncate sm:whitespace-normal">{item.subject}</td>
                      <td className="py-3 px-4 sm:px-6 whitespace-nowrap">
                        <span className={`text-[11px] font-bold px-2 py-0.5 rounded uppercase tracking-wide inline-block border ${PRIORITY_BADGES[item.priority]}`}>
                          {item.priority}
                        </span>
                      </td>
        ...
        <div className="lg:col-span-1 flex flex-col gap-5 sm:gap-6">
          ...
          <div className="bg-white border border-gray-300 rounded p-4 sm:p-6 shadow-sm flex flex-col justify-between">
            <div>
              <h2 className="text-base sm:text-lg font-bold text-gray-900 mb-4 sm:mb-6 uppercase tracking-wide">Departmental Health</h2>
                  ...
                  departmentHealth?.map(dept => (
                    <div key={dept.id}>
                      <div className="flex justify-between items-center mb-1.5 gap-2">
                        <span className="text-xs sm:text-sm font-semibold text-gray-800 truncate">{dept.department}</span>
                        <span className={`text-xs sm:text-sm font-bold shrink-0 ${dept.textColor}`}>{dept.loadPercent}% Load</span>
                      </div>
          ...
          {/* Quick Reports Card */}
          <div className="bg-white border border-gray-300 rounded p-4 sm:p-6 shadow-sm">
            <h2 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4 uppercase tracking-wide">Quick Reports</h2>
            <div className="space-y-2.5 sm:space-y-3">
              {loading ? (
                ...
                quickReports?.map(report => (
                  <div
                    key={report.id}
                    className="border border-gray-300 rounded p-3 sm:p-4 flex items-center gap-3 sm:gap-4 hover:border-[#A31736] hover:bg-gray-50 transition-all cursor-pointer group"
                  >
                    <div className="bg-[#A31736] text-white w-9 h-9 sm:w-10 sm:h-10 rounded flex items-center justify-center shrink-0 shadow-sm">
                      {report.icon === 'summary' ? <SummaryReportIcon /> : <FinancialReportIcon />}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs sm:text-sm font-bold text-gray-900 group-hover:text-[#A31736] transition-colors truncate">
                        {report.title}
                      </p>
                      <p className="text-[11px] sm:text-xs text-gray-500 mt-0.5 truncate">{report.subtitle}</p>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 border border-slate-700 rounded p-4 sm:p-6 shadow-md text-white flex flex-col justify-between">
            <div>
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                ...
                <span className="text-[11px] sm:text-xs text-slate-400">Pradeshiya Sabha Fleet</span>
              </div>
              <h3 className="text-base sm:text-lg font-bold text-white">Fleet Management System</h3>
              <p className="text-xs text-slate-300 mt-1 sm:mt-1.5 leading-relaxed">
            ...
            <div className="mt-5 pt-4 border-t border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <span className="text-xs text-slate-300">Council Transport</span>
              <a
                href="/fleet/overview"
                className="w-full sm:w-auto text-center px-4 py-2 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs rounded transition-all shadow-sm"
              >

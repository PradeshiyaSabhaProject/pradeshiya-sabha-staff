...
const ITEM_STATUS_STYLES: Record<ItemStatus, string> = {
  ...
  'Out of Stock': 'bg-red-50 text-[#801028] border-red-200',
}

const APPROVAL_STATUS_STYLES: Record<ApprovalStatus, string> = {
  'Pending Approval': 'bg-amber-50 text-amber-700 border-amber-200',
  Approved: 'bg-emerald-50 text-emerald-700 border-emerald-200',
  Rejected: 'bg-red-50 text-[#801028] border-red-200',
}
...
export const ItemStatusBadge: React.FC<{ status: ItemStatus }> = ({ status }) => (
  <span
    className={`text-[11px] px-2.5 py-0.5 rounded-full border uppercase tracking-wider font-bold whitespace-nowrap ${ITEM_STATUS_STYLES[status]}`}
  >
    {status}
  </span>
)
...
export const ApprovalStatusBadge: React.FC<{ status: ApprovalStatus }> = ({ status }) => (
  <span
    className={`text-[11px] px-2.5 py-0.5 rounded-full border uppercase tracking-wider font-bold whitespace-nowrap ${APPROVAL_STATUS_STYLES[status]}`}
  >
    {status}
  </span>
)
